	...
	setTimeout(function(){
		...
		if (window.self != window.top) {
			...
			if (this.contentWindow.document == window.document) {
					...
					if(document.body.scrollWidth - 15 > $(ifrm).width()) $(ifrm).css({'width':document.body.scrollWidth + 'px'});
				}
			});
		}	
...
function coolRadioCheckBtn( container ){
	var mycontainer = (typeof container == 'undefined') ? $('body') : container;

	//Personalizando los controles radio
	//console.log(mycontainer.find('.ewItemList'));

	mycontainer.find('.ewItemList').addClass('btn-group').attr('data-toggle','buttons');
	setTimeout(function(){
		mycontainer.find('.radio-inline,.checkbox-inline').addClass('btn alert-success').find('input:radio,input:checkbox').css({'width':'0px'}).filter(':checked').parent().addClass('active').end().end().end().css({'visibility':'inherit'});

		//console.log(	mycontainer.find('.checkbox-inline:not(.cool-ctrl)'));
		mycontainer.find('.checkbox-inline:not(.cool-ctrl)').on('click', function(event ){ 

...
$geoprocesamiento_edit = NULL; // Initialize page object first

class cgeoprocesamiento_edit extends cgeoprocesamiento {
	...
	// Page object name
	var $PageObjName = 'geoprocesamiento_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->idgeoproceso->CurrentValue == "")
			$this->Page_Terminate("geoprocesamientolist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->idgeoproceso->FldIsDetailKey)
			$this->idgeoproceso->setFormValue($objForm->GetValue("x_idgeoproceso"));
		...
		if (!$this->inicio->FldIsDetailKey) {
			...
			$this->inicio->CurrentValue = ew_UnFormatDateTime($this->inicio->CurrentValue, 0);
		}
		if (!$this->fin->FldIsDetailKey) {
			$this->fin->setFormValue($objForm->GetValue("x_fin"));
			$this->fin->CurrentValue = ew_UnFormatDateTime($this->fin->CurrentValue, 0);
		}
		if (!$this->entradatxt->FldIsDetailKey) {
			$this->entradatxt->setFormValue($objForm->GetValue("x_entradatxt"));
		}
		if (!$this->salidatxt->FldIsDetailKey) {
			$this->salidatxt->setFormValue($objForm->GetValue("x_salidatxt"));
		}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->idgeoproceso->CurrentValue = $this->idgeoproceso->FormValue;
		...
		$this->salidatxt->CurrentValue = $this->salidatxt->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// entradatxt
			$this->entradatxt->SetDbValueDef($rsnew, $this->entradatxt->CurrentValue, NULL, $this->entradatxt->ReadOnly);

			// opcionestxt
			$this->opcionestxt->SetDbValueDef($rsnew, $this->opcionestxt->CurrentValue, NULL, $this->opcionestxt->ReadOnly);
...
// Create page object
if (!isset($geoprocesamiento_edit)) $geoprocesamiento_edit = new cgeoprocesamiento_edit();

// Page init
$geoprocesamiento_edit->Page_Init();

// Page main
$geoprocesamiento_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$geoprocesamiento_edit->Page_Render();
?>
...
var CurrentForm = fgeoprocesamientoedit = new ew_Form("fgeoprocesamientoedit", "edit");

// Validate form
fgeoprocesamientoedit.Validate = function() {
...
fgeoprocesamientoedit.Form_CustomValidate = 
...
fgeoprocesamientoedit.ValidateRequired = true;
<?php } else { ?>
fgeoprocesamientoedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fgeoprocesamientoedit.Lists["x_idusuario"] = {"LinkField":"x_idusuario","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fgeoprocesamientoedit.Lists["x_proceso"] = {"LinkField":"x_idaccion","Ajax":true,"AutoFill":false,"DisplayFields":["x_idaccion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php $geoprocesamiento_edit->ShowPageHeader(); ?>
<?php
$geoprocesamiento_edit->ShowMessage();
?>
<form name="fgeoprocesamientoedit" id="fgeoprocesamientoedit" class="<?php echo $geoprocesamiento_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($geoprocesamiento_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $geoprocesamiento_edit->Token ?>">
<?php } ?>
...
<?php if ($geoprocesamiento->idgeoproceso->Visible) { // idgeoproceso ?>
	<div id="r_idgeoproceso" class="form-group">
		<label id="elh_geoprocesamiento_idgeoproceso" class="col-sm-2 control-label ewLabel"><?php echo $geoprocesamiento->idgeoproceso->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $geoprocesamiento->idgeoproceso->CellAttributes() ?>>
<span id="el_geoprocesamiento_idgeoproceso">
<span<?php echo $geoprocesamiento->idgeoproceso->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $geoprocesamiento->idgeoproceso->EditValue ?></p></span>
</span>
<input type="hidden" data-table="geoprocesamiento" data-field="x_idgeoproceso" name="x_idgeoproceso" id="x_idgeoproceso" value="<?php echo ew_HtmlEncode($geoprocesamiento->idgeoproceso->CurrentValue) ?>">
<?php echo $geoprocesamiento->idgeoproceso->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($geoprocesamiento->idusuario->Visible) { // idusuario ?>
	<div id="r_idusuario" class="form-group">
		<label id="elh_geoprocesamiento_idusuario" class="col-sm-2 control-label ewLabel"><?php echo $geoprocesamiento->idusuario->FldCaption() ?></label>
...
<p class="form-control-static"><?php echo $geoprocesamiento->idusuario->EditValue ?></p></span>
</span>
<input type="hidden" data-table="geoprocesamiento" data-field="x_idusuario" name="x_idusuario" id="x_idusuario" value="<?php echo ew_HtmlEncode($geoprocesamiento->idusuario->CurrentValue) ?>">
...
<?php if ($geoprocesamiento->inicio->Visible) { // inicio ?>
	<div id="r_inicio" class="form-group">
		<label id="elh_geoprocesamiento_inicio" for="x_inicio" class="col-sm-2 control-label ewLabel"><?php echo $geoprocesamiento->inicio->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $geoprocesamiento->inicio->CellAttributes() ?>>
<span id="el_geoprocesamiento_inicio">
<span<?php echo $geoprocesamiento->inicio->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $geoprocesamiento->inicio->EditValue ?></p></span>
</span>
<input type="hidden" data-table="geoprocesamiento" data-field="x_inicio" name="x_inicio" id="x_inicio" value="<?php echo ew_HtmlEncode($geoprocesamiento->inicio->CurrentValue) ?>">
<?php echo $geoprocesamiento->inicio->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($geoprocesamiento->fin->Visible) { // fin ?>
	<div id="r_fin" class="form-group">
		<label id="elh_geoprocesamiento_fin" for="x_fin" class="col-sm-2 control-label ewLabel"><?php echo $geoprocesamiento->fin->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $geoprocesamiento->fin->CellAttributes() ?>>
<span id="el_geoprocesamiento_fin">
<span<?php echo $geoprocesamiento->fin->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $geoprocesamiento->fin->EditValue ?></p></span>
</span>
<input type="hidden" data-table="geoprocesamiento" data-field="x_fin" name="x_fin" id="x_fin" value="<?php echo ew_HtmlEncode($geoprocesamiento->fin->CurrentValue) ?>">
<?php echo $geoprocesamiento->fin->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if ($geoprocesamiento->salidatxt->Visible) { // salidatxt ?>
	<div id="r_salidatxt" class="form-group">
		<label id="elh_geoprocesamiento_salidatxt" for="x_salidatxt" class="col-sm-2 control-label ewLabel"><?php echo $geoprocesamiento->salidatxt->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $geoprocesamiento->salidatxt->CellAttributes() ?>>
<span id="el_geoprocesamiento_salidatxt">
<span<?php echo $geoprocesamiento->salidatxt->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $geoprocesamiento->salidatxt->EditValue ?></p></span>
</span>
<input type="hidden" data-table="geoprocesamiento" data-field="x_salidatxt" name="x_salidatxt" id="x_salidatxt" value="<?php echo ew_HtmlEncode($geoprocesamiento->salidatxt->CurrentValue) ?>">
<?php echo $geoprocesamiento->salidatxt->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<span<?php echo $geoprocesamiento->geojson->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $geoprocesamiento->geojson->EditValue ?></p></span>
</span>
<input type="hidden" data-table="geoprocesamiento" data-field="x_geojson" name="x_geojson" id="x_geojson" value="<?php echo ew_HtmlEncode($geoprocesamiento->geojson->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $geoprocesamiento_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
fgeoprocesamientoedit.Init();
</script>
<?php
$geoprocesamiento_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$geoprocesamiento_edit->Page_Terminate();
?>

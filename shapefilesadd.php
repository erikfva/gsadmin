...
class cshapefiles_add extends cshapefiles {
	...
	function __construct() {
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'shapefiles');
	...
	function LoadDefaultValues() {
		...
		$this->acciones->CurrentValue = NULL;
		$this->acciones->OldValue = $this->acciones->CurrentValue;
	}
	...
	function ValidateForm() {
		...
		if (!EW_SERVER_VALIDATE)
			return ($gsFormError == "");
		if ($this->srid->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->srid->FldCaption(), $this->srid->ReqErrMsg));
		}
	...
	function AddRow($rsold = NULL) {
		...
		// acciones
		$this->acciones->SetDbValueDef($rsnew, $this->acciones->CurrentValue, NULL, FALSE);
		if ($this->narchivo->Visible && !$this->narchivo->Upload->KeepFile) {
	...
	function Page_Load() {
		...
		if(chkopt("webservice")){ //Si se ha llamado como servicio.
			if(isset($_FILES['uploadedfile']) && $_FILES['uploadedfile']['error'] == 0 && IsLoggedIn() ){

			//-----------------------------------//
			//***Completando variables requeridas enviadas por POST***
			//-----------------------------------//

				$filedata = file_get_contents($_FILES["uploadedfile"]["tmp_name"]); //leyendo el contenido del archivo desde almac. temporal
				$_POST["x_filename"] = $_FILES["uploadedfile"]["name"]; //nombre del archivo en el cliente.
				$_POST["file"] = $_FILES["uploadedfile"]; //Informacion referencial del archivo cargado.
				$_POST["x_filedata"] = $filedata; //Stream del contenido del archivo.
				$_POST["x_srid"] = $_POST["proj"];//Codigo de sistema de proyeccion del archivo shape

				//$_SESSION[CurrentPage()->PageObjName."_opciones"] .= ",allow_polygon,fix_si,geojson";
				//opciones para el script del servidor:
...
fshapefilesadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			elm = this.GetElements("x" + infix + "_srid");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $shapefiles->srid->FldCaption(), $shapefiles->srid->ReqErrMsg)) ?>");
...
fshapefilesadd.Lists["x_acciones[]"] = {"LinkField":"x_idaccion","Ajax":true,"AutoFill":false,"DisplayFields":["x_accion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php if ($shapefiles->srid->Visible) { // srid ?>
	<div id="r_srid" class="form-group">
		<label id="elh_shapefiles_srid" class="col-sm-2 control-label ewLabel"><?php echo $shapefiles->srid->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>

...
// Clean output buffer
        ...
        if (@$_SESSION[CurrentPage()->PageObjName . "_WSR"]) {
            echo json_encode(@$_SESSION[CurrentPage()->PageObjName . "_WSR"]);
        }
        //$strJSON;
        exit();
    }
...
if (!defined('UPLOADS_DIR')) {
    define("UPLOADS_DIR", dirname(realpath(__FILE__)) . DIRECTORY_SEPARATOR . "jupload" . DIRECTORY_SEPARATOR . "server" . DIRECTORY_SEPARATOR . "php" . DIRECTORY_SEPARATOR . "files" . DIRECTORY_SEPARATOR);
}

if (!defined('DATA_URL')) {
    define("DATA_URL", get_full_url() . (strpos(get_full_url(), "jupload") > -1 ? '' : '/jupload') . "/server/php/");
}

//Definiendo la url del mapserver
if (!defined("MSRV_URL")) {
    define("MSRV_URL", ew_DomainUrl());
}
...
<script type=\"text/javascript\">
	...
	function loadOpenlayersScript(f) {
        ...
        case "UIDATETIME":
            ...
            break;
        case "JQUERYUI":
            if ($posicion == "header") {ew_AddStylesheet("ui/themes/cupertino/jquery-ui.min.css");} else {ew_AddClientScript("ui/jquery-ui-1.10.0.custom.min.js", array("requiere" => $EW_RELATIVE_PATH . "jquery/jquery-1.11.3.min.js"));}
            break;
        case "UIDIALOG":
            if ($posicion == "header") {ew_AddStylesheet("uidialog/jquery.dialogr.css");} else {ew_AddClientScript("uidialog/ui.dialogr.min.js", array("requiere" => "jquery-ui-1.10.0.custom.min.js"));}
            break;
        case "LIMESURVEY":
            if ($posicion == "footer") {
...
<script type=\"text/javascript\">
	 jQuery(document).ready(function(){
		...
		";
            ...
            break;
        case "TIMEPICKER":
            if ($posicion == "header") {ew_AddStylesheet("timepicker/jquery-ui-timepicker-addon.css");} else {

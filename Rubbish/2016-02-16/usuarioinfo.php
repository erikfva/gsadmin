...
class cusuario extends cTable {
	...
	function __construct() {
		...
		// password
		$this->password = new cField('usuario', 'usuario', 'x_password', 'password', '"password"', '"password"', 200, -1, FALSE, '"password"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// perfil
		$this->perfil = new cField('usuario', 'usuario', 'x_perfil', 'perfil', '"perfil"', '"perfil"', 200, -1, FALSE, '"perfil"', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
   ...
   // Common render codes
	...
	function GetAutoFill($id, $val) {
		$rsarr = array();
		$rowcnt = 0;
		if (preg_match('/^x(\d)*_userlevelid$/', $id)) {
			$conn = &$this->Connection();
			$sSqlWrk = "SELECT \"perfil\" AS FIELD0 FROM \"registro_derecho\".\"userlevels\"";
			$sWhereWrk = "(\"userlevelid\" = " . ew_QuotedValue($val, EW_DATATYPE_NUMBER, $this->DBID) . ")";
			$this->Lookup_Selecting($this->userlevelid, $sWhereWrk); // Call Lookup selecting
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			$sSqlWrk .= " ORDER BY \"userlevelname\"";
			if ($rs = ew_LoadRecordset($sSqlWrk, $conn)) {
				while ($rs && !$rs->EOF) {
					$ar = array();
					$this->perfil->setDbValue($rs->fields[0]);
					$this->RowType == EW_ROWTYPE_EDIT;
					$this->RenderEditRow();
					$ar[] = $this->perfil->CurrentValue;
					$rowcnt += 1;
					$rsarr[] = $ar;
					$rs->MoveNext();
				}

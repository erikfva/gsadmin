...
class cusuario_edit extends cusuario {
	...
	function LoadFormValues() {
		...
		if (!$this->autologinip->FldIsDetailKey) {
			$this->autologinip->setFormValue($objForm->GetValue("x_autologinip"));
		}
	}
	...
	function RestoreFormValues() {
		...
		$this->autologinip->CurrentValue = $this->autologinip->FormValue;
	}
	...
	function RenderRow() {
		...
		$this->autologinip->ViewCustomAttributes = "";
			...
			if (trim(strval($this->idperfil->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$arwrk = explode(",", $this->idperfil->CurrentValue);
				$sFilterWrk = "";
			...
			$arwrk = ($rswrk) ? $rswrk->GetRows() : array();
			if ($rswrk) $rswrk->Close();
			$this->idperfil->EditValue = $arwrk;

			// autologinip
			$this->autologinip->EditAttrs["class"] = "form-control";
			$this->autologinip->EditCustomAttributes = "";
			$this->autologinip->EditValue = ew_HtmlEncode($this->autologinip->CurrentValue);
			$this->autologinip->PlaceHolder = ew_RemoveHtml($this->autologinip->FldCaption());
	...
	function EditRow() {
		...
		if ($this->user->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(\"user\" = '" . ew_AdjustSql($this->user->CurrentValue, $this->DBID) . "')";
		...
		if ($rs->EOF) {
			...
			// idperfil
			$this->idperfil->SetDbValueDef($rsnew, $this->idperfil->CurrentValue, 0, $this->idperfil->ReadOnly);

			// autologinip
			$this->autologinip->SetDbValueDef($rsnew, $this->autologinip->CurrentValue, NULL, $this->autologinip->ReadOnly);
...
fusuarioedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $usuario->idperfil->FldCaption(), $usuario->idperfil->ReqErrMsg)) ?>");
...
fusuarioedit.Lists["x_activo"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fusuarioedit.Lists["x_activo"].Options = <?php echo json_encode($usuario->activo->Options()) ?>;
fusuarioedit.Lists["x_idperfil[]"] = {"LinkField":"x_idperfil","Ajax":true,"AutoFill":false,"DisplayFields":["x_idperfil","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php if ($usuario->idusuario->Visible) { // idusuario ?>
	<div id="r_idusuario" class="form-group">
		<label id="elh_usuario_idusuario" class="col-sm-2 control-label ewLabel"><?php echo $usuario->idusuario->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $usuario->idusuario->CellAttributes() ?>>
...
<?php echo $usuario->idusuario->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if ($usuario->idperfil->Visible) { // idperfil ?>
	<div id="r_idperfil" class="form-group">
		<label id="elh_usuario_idperfil" class="col-sm-2 control-label ewLabel"><?php echo $usuario->idperfil->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $usuario->idperfil->CellAttributes() ?>>
<span id="el_usuario_idperfil">
<div id="tp_x_idperfil" class="ewTemplate"><input type="checkbox" data-table="usuario" data-field="x_idperfil" data-value-separator="<?php echo ew_HtmlEncode(is_array($usuario->idperfil->DisplayValueSeparator) ? json_encode($usuario->idperfil->DisplayValueSeparator) : $usuario->idperfil->DisplayValueSeparator) ?>" name="x_idperfil[]" id="x_idperfil[]" value="{value}"<?php echo $usuario->idperfil->EditAttributes() ?>></div>
<div id="dsl_x_idperfil" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php
$arwrk = $usuario->idperfil->EditValue;
if (is_array($arwrk)) {
	$armultiwrk = (strval($usuario->idperfil->CurrentValue) <> "") ? explode(",", strval($usuario->idperfil->CurrentValue)) : array();
	...
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		...
		if ($selwrk <> "") {
?>
<label class="checkbox-inline"><input type="checkbox" data-table="usuario" data-field="x_idperfil" name="x_idperfil[]" id="x_idperfil_<?php echo $rowcntwrk ?>[]" value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?><?php echo $usuario->idperfil->EditAttributes() ?>><?php echo $usuario->idperfil->DisplayValue($arwrk[$rowcntwrk]) ?></label>
<?php
		}
	}
	for ($ari = 0; $ari < $cnt; $ari++) {
...
?>
<label class="checkbox-inline"><input type="checkbox" data-table="usuario" data-field="x_idperfil" name="x_idperfil[]" value="<?php echo ew_HtmlEncode($armultiwrk[$ari]) ?>" checked<?php echo $usuario->idperfil->EditAttributes() ?>><?php echo $armultiwrk[$ari] ?></label>
<?php
		}
	}
}
...
$sSqlWrk = "SELECT \"idperfil\", \"idperfil\" AS \"DispFld\", '' AS \"Disp2Fld\", '' AS \"Disp3Fld\", '' AS \"Disp4Fld\" FROM \"registro_derecho\".\"perfil\"";
$sWhereWrk = "";
$usuario->idperfil->LookupFilters = array("s" => $sSqlWrk, "d" => "");
$usuario->idperfil->LookupFilters += array("f0" => "\"idperfil\" = {filter_value}", "t0" => "3", "fn0" => "");
$sSqlWrk = "";
$usuario->Lookup_Selecting($usuario->idperfil, $sWhereWrk); // Call Lookup selecting
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
if ($sSqlWrk <> "") $usuario->idperfil->LookupFilters["s"] .= $sSqlWrk;
?>
<input type="hidden" name="s_x_idperfil" id="s_x_idperfil" value="<?php echo $usuario->idperfil->LookupFilterQuery() ?>">
</span>
<?php echo $usuario->idperfil->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($usuario->autologinip->Visible) { // autologinip ?>
	<div id="r_autologinip" class="form-group">
		<label id="elh_usuario_autologinip" for="x_autologinip" class="col-sm-2 control-label ewLabel"><?php echo $usuario->autologinip->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $usuario->autologinip->CellAttributes() ?>>
<span id="el_usuario_autologinip">
<input type="text" data-table="usuario" data-field="x_autologinip" name="x_autologinip" id="x_autologinip" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($usuario->autologinip->getPlaceHolder()) ?>" value="<?php echo $usuario->autologinip->EditValue ?>"<?php echo $usuario->autologinip->EditAttributes() ?>>
</span>
<?php echo $usuario->autologinip->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>

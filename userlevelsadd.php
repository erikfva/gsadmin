...
fuserlevelsadd.Lists["x_idperfil"] = {"LinkField":"x_idperfil","Ajax":true,"AutoFill":false,"DisplayFields":["x_idperfil","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
<?php if ($userlevels->idperfil->Visible) { // idperfil ?>
	<div id="r_idperfil" class="form-group">
		<label id="elh_userlevels_idperfil" for="x_idperfil" class="col-sm-2 control-label ewLabel"><?php echo $userlevels->idperfil->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $userlevels->idperfil->CellAttributes() ?>>
<span id="el_userlevels_idperfil">
<select data-table="userlevels" data-field="x_idperfil" data-value-separator="<?php echo ew_HtmlEncode(is_array($userlevels->idperfil->DisplayValueSeparator) ? json_encode($userlevels->idperfil->DisplayValueSeparator) : $userlevels->idperfil->DisplayValueSeparator) ?>" id="x_idperfil" name="x_idperfil"<?php echo $userlevels->idperfil->EditAttributes() ?>>
<?php
if (is_array($userlevels->idperfil->EditValue)) {
	$arwrk = $userlevels->idperfil->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = ew_SameStr($userlevels->idperfil->CurrentValue, $arwrk[$rowcntwrk][0]) ? " selected" : "";
		if ($selwrk <> "") $emptywrk = FALSE;		
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $userlevels->idperfil->DisplayValue($arwrk[$rowcntwrk]) ?>
</option>
<?php
	}
	if ($emptywrk && strval($userlevels->idperfil->CurrentValue) <> "") {
?>
<option value="<?php echo ew_HtmlEncode($userlevels->idperfil->CurrentValue) ?>" selected><?php echo $userlevels->idperfil->CurrentValue ?></option>
<?php
    }
}
?>
...
$sSqlWrk = "SELECT \"idperfil\", \"idperfil\" AS \"DispFld\", '' AS \"Disp2Fld\", '' AS \"Disp3Fld\", '' AS \"Disp4Fld\" FROM \"registro_derecho\".\"perfil\"";
$sWhereWrk = "";
$userlevels->idperfil->LookupFilters = array("s" => $sSqlWrk, "d" => "");
$userlevels->idperfil->LookupFilters += array("f0" => "\"idperfil\" = {filter_value}", "t0" => "3", "fn0" => "");
$sSqlWrk = "";
$userlevels->Lookup_Selecting($userlevels->idperfil, $sWhereWrk); // Call Lookup selecting
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
if ($sSqlWrk <> "") $userlevels->idperfil->LookupFilters["s"] .= $sSqlWrk;
?>
<input type="hidden" name="s_x_idperfil" id="s_x_idperfil" value="<?php echo $userlevels->idperfil->LookupFilterQuery() ?>">
</span>
<?php echo $userlevels->idperfil->CustomMsg ?></div></div>
	</div>
<?php } ?>
